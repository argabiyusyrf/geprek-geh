...
class ProductController {
    ...
    public function create() {
        \Auth::requireStaff();
        gg_redirect('/admin/products?create=1');
        exit;
    }

    public function store() {
        \Auth::requireStaff();
        if (!\verify_csrf()) { \flash_set('error', 'Token tidak valid.'); gg_redirect('/admin/products?create=1'); exit; }
        $db = \Database::getInstance();
        ...
        if ($name === '') {
            ...
            gg_redirect('/admin/products?create=1&error=1'); exit;
        }
    ...
    public function edit($id) {
        ...
        gg_redirect('/admin/products?edit=' . (int) $id);
        exit;
    }
    ...
    public function update($id) {
        \Auth::requireStaff();
        $id = (int) $id;
        if (!\verify_csrf()) { \flash_set('error', 'Token tidak valid.'); gg_redirect('/admin/products'); exit; }
        $db = \Database::getInstance();
        ...
        if ($name === '') {
            \form_stash(['name' => 'Nama produk tidak boleh kosong.'], $stash);
            \flash_set('error', 'Mohon periksa kembali isian form produk.');
            gg_redirect('/admin/products?edit=' . $id . '&error=1'); exit;
        }
        if (!$db->fetchOne("SELECT id FROM categories WHERE id = ?", [$category_id])) {
            \form_stash(['category_id' => 'Kategori tidak valid.'], $stash);
            \flash_set('error', 'Mohon periksa kembali isian form produk.');
