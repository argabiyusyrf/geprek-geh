...
class HomeController {
    public function index() {
        ...
        $featured = $db->fetchAll("SELECT p.*, c.name AS category_name,
              ...
              LEFT JOIN (SELECT product_id, COUNT(*) AS review_count, AVG(rating) AS avg_rating FROM product_reviews WHERE is_visible = 1 GROUP BY product_id) r ON r.product_id = p.id
