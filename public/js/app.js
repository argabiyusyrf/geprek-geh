...
/* ── Checkout: address add drawer ── */
(function checkoutAddrDrawer() {
    const overlay = document.getElementById('checkout-addr-overlay');
    const drawer = document.getElementById('checkout-addr-drawer');
    const form = document.getElementById('checkoutAddrForm');
    if (!overlay || !drawer || !form) return;

    const openers = document.querySelectorAll('[data-open-address-drawer]');
    const closers = document.querySelectorAll('[data-close-checkout-addr]');
            ...
            .then((data) => {
                if (data && data.ok) {
                    ...
                    if (wrap && a) {
                        ...
                        card.className = 'sa-radio-card';
                        card.dataset.savedAddress = '';
                        card.dataset.id = a.id;
                        card.dataset.recipient = a.recipient_name;
                        card.dataset.phone = a.phone;
                        card.dataset.address = a.address;
                        card.innerHTML =
                            '<input type="radio" name="picked_address" value="' + a.id + '" class="sa-radio-input">' +
                            '<span class="sa-radio-dot"></span>' +
                            '<span class="sa-card-body">' +
                                '<span class="sa-label">' + (a.label || 'Alamat') + (a.is_default ? ' <span class="address-badge">Utama</span>' : '') + '</span>' +
                                '<span class="sa-name">' + a.recipient_name + '</span>' +
                                '<span class="sa-addr">' + a.address + '</span>' +
                            '</span>';

                        card.addEventListener('click', () => {
                            const ni = document.querySelector('input[name="recipient_name"]');
                            const pi = document.querySelector('input[name="phone"]');
                            const ai = document.querySelector('textarea[name="address"]');
                            const idi = document.getElementById('addressIdInput');
                            const fw = document.getElementById('checkoutFields');
                            if (ni) ni.value = a.recipient_name;
                            if (pi) pi.value = a.phone;
                            if (ai) ai.value = a.address;
                            if (idi) idi.value = a.id;
                            wrap.querySelectorAll('.sa-radio-card').forEach((c) => c.classList.remove('is-active'));
                            card.classList.add('is-active');
                            if (fw) fw.classList.add('is-collapsed');
                        });

                        const manual = wrap.querySelector('[data-saved-manual]');
                        if (manual) {
                            wrap.insertBefore(card, manual);
                        } else {
                    ...
                    alert((data && data.message) || 'Gagal menyimpan alamat.');
                }
            })
